...
// ---- LED strip -------------------------------------------------------------
constexpr uint16_t NUM_LEDS = 48;  // ~30cm of 160 LED/m FCOB
...
// loop clamps every frame. ~40% of full scale keeps current/heat in check.
constexpr uint8_t BRIGHTNESS_CAP = 100;

// User-cyclable brightness levels (low / medium / high). All must be <= cap.
constexpr uint8_t BRIGHTNESS_LEVELS[] = {25, 60, 100};
